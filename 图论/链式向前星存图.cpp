...
int cnt = 1;   //工具变量，记录边的序号，记录了有几条边，add函数用一次自加一次

void add(int u, int v, int w)  //输入从u到v的长度为w的边
{
    edge[cnt].w = w;  //第cnt条边长为w
    edge[cnt].to = v;  //第cnt条边指向点v
    edge[cnt].next = head[u]; //指向上一条边的位置
     //上一条以u为起点的最大序号的边的序号是head[u]，每一条边都记录下了上一个同起点的边的序号，用next
    head[u] = cnt++;  //更新起点u的存放位置
}
...
int main()
{
    ...
    {
        for (int j = head[i]; j != -1; j=edge[i].next)  //从以i为起点的最大序号边开始，通过每条边的next递推从而依次按边的序号从大到小输出边，到j=-1就说明输出完了
        {
