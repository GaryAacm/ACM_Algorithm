...
int askk(int l,int r)//查询区间
{
   int k=log2(r-l+1);//因为向下取整，所有会有重叠查询
   return min(ff[l][k],ff[r-(1<<k)+1][k]);//左右重叠查询,右端的r-(1<<k)+1是为了找到起点
}

int main()
{
    int n,m;
    //freopen("D:\\算法进阶\\ST表\\in.txt","r",stdin);
    n=read();
    m=read();
    //读入数据
    for(int i=1;i<=n;++i)
    ...
    for(int j=1;j<=21;++j)//枚举区间长度
    {
        for(int i=1;i+(1<<j)-1<=n;++i)//枚举起点，j表示后面的数
        {
            ff[i][j]=min(ff[i][j-1],ff[(1<<(j-1))+i][j-1]);//第二位j是二的几次方表示
        }
    }
